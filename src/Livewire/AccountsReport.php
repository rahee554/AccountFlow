...
class AccountsReport extends Component
{
    ...
    private function getCategoriesWithTotalAmount()
    {
        ...
        $categories = $categories->map(function ($category) use ($categorySums) {
            ...
            $category->setAttribute('total_income', $income);
            $category->setAttribute('total_expense', $expense);
            // net is income minus expense
            $category->setAttribute('net', $income - $expense);
    ...
    private function getTotalAmount()
    {
        ...
        $row = DB::table('ac_transactions')
            ->select(DB::raw('SUM(CASE WHEN type = 1 THEN amount ELSE 0 END) as total_income'), DB::raw('SUM(CASE WHEN type = 2 THEN amount ELSE 0 END) as total_expense'))
            ->whereBetween('date', [$this->startDate, $this->endDate])
            ->first();
