...
class CreateCategory extends Component
{
    ...
    {
        $this->authorizeAccountFlow(Ability::ManageCategories);
        if ($id) {
            // Decode the base64-encoded id if needed
            if (! is_numeric($id)) {
                $id = \ArtflowStudio\AccountFlow\Support\RouteKey::decode($id);
            }
        }
        if ($id) {
            $this->isEdit = true;
            ...
            $this->validate([
                'parent_id' => [
                    ...
                    function ($attribute, $value, $fail) {
                        ...
                        if ($selectedParent && (int) $selectedParent->parent_id === (int) $this->categoryId) {
