...
class CreatePlannedPayment extends Component
{
    ...
    {
        $this->authorizeAccountFlow(Ability::ManagePlannedPayments);
    ...
    public function save()
    {
        ...
        // The form collects a due date and a "recurring" flag; the table
        // stores a window (start_date / end_date) plus a schedule_type. The old
        // code wrote the form's names straight through — `due_date`, `period`,
        // `auto_post_date`, `recurring` and `account_id` are not columns, so
        // every save failed with "Unknown column 'due_date'".
        $startDate = $this->due_date ?: now()->toDateString();
        $scheduleType = $this->recurring ? ($this->schedule_type ?: 'monthly') : ScheduleType::Once->value;
        ...
        PlannedPayment::create([
            ...
            // The scheduler posts on this date and then advances it.
